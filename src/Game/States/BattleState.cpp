...
void BattleState::continueBattle(Enemy *enemy){
    this->enemy = enemy;
    player->setHealth(getCurrentPlayerHealth());
}
...
void BattleState::update() {
    if (canInteract) {
        if (currentPlayerHealth <= 0) {
            ...
            return;
        } else if (currentEnemyHealth <= 0 && enemy->hasAnotherHealthBar()) {
            enemy->setAnotherHealthBar(false);
